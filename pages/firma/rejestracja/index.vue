        ...
        <div id="register-box">
            ...
            <form @submit.prevent="Register" method="post">
                ...
                <div>
                    ...
                    <input :class="{ 'input-error': error.nipErr }" placeholder="NIP" type="text" name="nip" v-model="data.nip">
                    <span class="error">{{ error.nipErr }}</span>
                </div>
            ...
            <div id="login">
                <p>Masz już konto?</p>
                <Nuxt-link to="/firma/logowanie">Zaloguj się</Nuxt-link>
            </div>
        </div>
    </div>
</template>
...
<script setup>
    ...
    function GoToLogin(email) {
        ...
        router.push({ path: '/firma/logowanie', query: { email: data.value.email } });
    }
    ...
    async function Register() {
        ...
        if(data.value.nip == '') {
            error.value.nipErr = 'Wypełnij pole!';
            success.value = false;
        } else {
            error.value.nipErr = '';
            success.value = true;
        ...
        if(success.value === true) {
            const response = await $fetch('http://localhost/advertising-system/backend/api/company/RegisterCompany.php' , { body: data.value, method: 'post', responseType: 'json' });
